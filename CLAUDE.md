...
**Auth gate.** `src/proxy.ts` is the Next 16 proxy (formerly middleware — same role, new filename/export). It runs `updateSession` on every non-static request, which calls `supabase.auth.getClaims()` and **redirects any unauthenticated request to `/login`** unless the path starts with `/login` or `/auth`. Neither route exists yet, so the app currently redirect-loops for signed-out users — adjust the matcher or add those routes when working on pages. Never insert code between `createServerClient` and `getClaims()` in `src/lib/supabase/proxy.ts`, and return the `supabaseResponse` object unmodified (cookie sync).

**UI.** shadcn/ui with the `base-luma` style on top of `@base-ui/react` (not Radix), icons from `@phosphor-icons/react`, theme tokens in `src/app/globals.css` via `@theme inline`. Add components with `bunx shadcn@latest add <name>` so `components.json` aliases apply.
